...
export const onRequest = methodRouter({
  ...
  async PUT(context) {
    ...
    await db.prepare(`
      UPDATE enquiries SET enquiry_type = ?, customer_name = ?, contact_email = ?,
        contact_phone = ?, job_details = ?, location = ?, preferred_date = ?,
        project_start_date = ?, project_end_date = ?, status = ?,
        dispatcher_notes = ?, is_trashed = ?, anticipated_hours = ?,
        site_inspection_required = ?, asset_type_id = ?, asset_requirement = ?,
        po_number = ?, updated_at = ?
      WHERE id = ?
    `).bind(
      e.enquiry_type, e.customer_name, e.contact_email, e.contact_phone ?? null,
      e.job_details ?? null, e.location ?? null, e.preferred_date ?? null,
      e.project_start_date ?? null, e.project_end_date ?? null, e.status,
      e.dispatcher_notes ?? null, e.is_trashed ? 1 : 0,
